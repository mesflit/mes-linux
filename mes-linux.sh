...
PROGRAM_VERSION="8.8.8"

#############################################################################
##
## >>> CONTRIBUTOR'S NOTICE <<<
##
## By contributing to the mes-linux project you are obligated to follow
## the next mandatory rules:
##
## 1. The mes-linux project is built around PRoot and is intended to be
##    developed as a wrapper around PRoot. Supporting classical Linux chroot
##    is outside of the project scope.
##
## 2. The mes-linux is designed to work only in Termux. Supporting other
##    distributions where PRoot is available is out of scope.
##
## 3. The mes-linux is intended to be used by a non-root user. Using it
...
##    using mes-linux by the root user.
##
## 4. mes-linux is not a generic tool. Its idea is to provide user a
...
##    Therefore mes-linux does not provide a way to modify its behavior.
##
## ***
##
...
##    how to restructure the mes-linux, then please keep them in your
##    own fork! Source customization is not welcome.
##
## 5.1 mes-linux is written in Bash script language with using features
...
##
##     * Commands: implement specific features of mes-linux such as
...
##       output describing usage of mes-linux commands. These functions
...
##     a brief overview which part of mes-linux is implemented here. The
...
##
## 5.7 Global variables must be defined either at beginning of mes-linux
...
##      during installation of the mes-linux. Such configuration is
##      treated as persistent and should not be changed by user.
##
## 5.14 mes-linux uses string place holders to define certain common
...
##
## 6.3 All messages printable by mes-linux script must support colored
##     text through the escape sequences defined below by variables such as
##     RED (red), BRED (bold red text), YELLOW, BYELLOW, etc.
##
## 6.4 mes-linux operation errors must have bold red color (${BRED}).
...
##
## 6.7 Informational messages produced by mes-linux command steps must
...
##
## 7.1 mes-linux script is intended to be distribution-agnostic. That
...
##     plug-in is a Bash script that is sourced by mes-linux and has the
##     following format:
##
##     DISTRO_NAME="Example"
...
##     has access to all variables defined by mes-linux during the
...
##
## 7.8 The alias files are located in $PREFIX/etc/mes-linux and have
...
##
## 7.9 The alias for distribution must be unique and mes-linux should
...
##
## 9.1 mes-linux has been created with aim to solve a specific range
##     of tasks and its functionality may not align with your needs.
##
## 9.2 mes-linux is a Bash script wrapper for PRoot. If PRoot does not
##     work on your device, the mes-linux will not work here as well.
##     You shall not beg the authors of mes-linux to fix the PRoot. Such
##     requests will be ignored.
##
## 9.3 Certain functionality of mes-linux relies on the Internet and
...
##     of the mes-linux.
##
## 9.5 mes-linux authors make the final decision whether to accept
##     submitted patches (pull requests) or not.
##
## 9.6 mes-linux authors do not provide the time frames for fullfilling
##     a specific request (new feature, etc).
##
## 9.7 mes-linux does not have upgrade schedule. Authors work on the
...
PROGRAM_NAME="mes-linux"

# Where distribution plug-ins are stored.
DISTRO_PLUGINS_DIR="@TERMUX_PREFIX@/etc/mes-linux"

# Base directory where script keeps runtime data.
RUNTIME_DIR="@TERMUX_PREFIX@/var/lib/mes-linux"
...
# Note: faking kernel version is required when using mes-linux on
# old devices that are not compatible with up-to-date versions of GNU libc.
DEFAULT_FAKE_KERNEL_VERSION="6.2.1-mes-linux"
...
# The supplied tarball should be one made by mes-linux as it has a proper
...
command_restore() {
	...
	if $success; then
		...
		msg
		msg "${BRED}The file is probably corrupted.${RST}"
		msg
	fi
}
...
#
# Prints a description of mes-linux utility and list of the available
# commands.
#
#############################################################################
...
command_help() {
	...
	msg
	msg "${CYAN}mes-linux is a Bash script wrapper for PRoot. It provides${RST}"
...
show_version() {
	msg "${ICYAN}mes-linux v${PROGRAM_VERSION} by Mesflit.${RST}"
}
